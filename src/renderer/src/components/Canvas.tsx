...
interface CanvasProps {
  ...
  onFocusTerminal: (terminalWindowId: string) => void;
}
...
export function Canvas(props: CanvasProps) {
  ...
  function addTerminalAtCenter(): void {
    const rect = frameRef.current?.getBoundingClientRect();
    if (!rect) {
      // Focus mode hides the canvas frame, so we can't read its rect.
      // Spawn with default size at the origin — the infinite canvas will
      // center it when the user toggles focus mode off.
      props.onAddTerminal({
        x: 0,
        y: 0,
  ...
  const focusMode = props.workspace.focusMode;
  const focusToggle = (
    <button
      type="button"
      ...
      title={focusMode ? "Exit focused mode" : "Enter focused mode"}
    >
      <span className="focus-mode-toggle-dot" aria-hidden="true">
    ...
    <section className="relative flex-1 min-w-0 min-h-0 grid grid-rows-[1fr]">
      ...
      <div
        className={`absolute left-1/2 -translate-x-1/2 z-30 pointer-events-auto ${
          focusMode
            ? "top-4 max-md:top-2.5"
            : "top-4 max-md:top-auto max-md:bottom-3"
        }`}
        aria-label="Focused mode"
      >
        {focusToggle}
      </div>
      ...
      {focusMode ? (
        <FocusModeView
          terminals={props.workspace.terminals}
          focusedTerminalId={props.workspace.focusedTerminalId}
          workspaceSettings={props.workspace.settings}
          workspaceName={props.workspace.name}
          appDefaultShellId={props.appDefaultShellId}
          onFocusChange={props.onFocusTerminal}
          onPatchTerminal={props.onPatchTerminal}
